...
""",
    ...
    # --- Experimental verbs (0.13.0) -------------------------------------
    # These are intentionally additive and tagged [EXPERIMENTAL] in the
    # cheatsheet/help text. They may rename or vanish between minors.
    try:
        from multiligua_cli.experimental import add_subparsers as _add_exp_parsers
        _add_exp_parsers(sub)
    ...
    p_skill = sub.add_parser("skill", help="Manage MINXG skills (markdown instruction bundles)")
    ...
    p_skill_list.add_argument("--category", help="Filter by category")
    p_skill_list.add_argument("--installed", action="store_true",
                               help="Show only user-installed skills")

    p_skill_view = skill_sub.add_parser("view", help="View a skill's full content")
    p_skill_view.add_argument("name")

    p_skill_search = skill_sub.add_parser("search", help="Search a skill catalog")
    p_skill_search.add_argument("query", nargs="?", default="")
    p_skill_search.add_argument("--tag", help="Filter by tag")
    p_skill_search.add_argument("--catalog", help="Catalog URL or file path (default: bundled)")

    p_skill_install = skill_sub.add_parser(
        "install", help="Install from a local path, git URL, raw URL, or catalog name")
    p_skill_install.add_argument("source")
    p_skill_install.add_argument("--yes", action="store_true",
                                  help="Actually write files (previews without this)")
    p_skill_install.add_argument("--as", dest="as_name",
                                  help="Install under a different name")

    p_skill_new = skill_sub.add_parser("new", help="Scaffold a new skill")
    p_skill_new.add_argument("name")
    p_skill_new.add_argument("--description", default="")
    p_skill_new.add_argument("--author", default="")

    p_skill_remove = skill_sub.add_parser("remove", help="Uninstall a user skill")
    p_skill_remove.add_argument("name")

    p_skill_publish = skill_sub.add_parser(
    ...
    # subcommands failed with "invalid choice" even when enabled.
    from extensions import register_cli_extensions
    ext_cli_map = register_cli_extensions(sub)
    ...
    if cmd in ext_cli_map:
        from extensions import dispatch_extension
        return dispatch_extension(ext_cli_map, cmd, args)
    ...
    if getattr(args, "_experimental_cmd", None):
        from multiligua_cli.experimental import dispatch as _exp_dispatch
        return _exp_dispatch(args)
    ...
    if cmd == "skill":
        skill_act = getattr(args, "skill_action", None)
        from multiligua_cli.skill_cli import dispatch_skill_command
        return dispatch_skill_command(args, skill_act)

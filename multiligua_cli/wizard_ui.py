"""
MINXG Wizard UI Engine v3.0 — high-density native TUI.
...
def print_chat_banner() -> None:
    ...
    if HAS_RICH:
        ...
        banner_text.append(
            f"\n  v{ver}  MINXG Chat — interactive REPL\n",
            style="italic dim cyan",
        )

        notice_lines = [
            "[bold bright_white]NO WARRANTY[/bold bright_white]  —  "
            "AS IS, MIT License. No fitness guarantee.",
            "[bold bright_white]NO LEGAL ADVICE[/bold bright_white]  —  "
            "Informational only. Not legal advice. "
            "EU 2009/24/EC Art.6 / US DMCA §1201(f).",
            "[bold bright_white]DISCLAIMER OF AGENCY[/bold bright_white]  —  "
    ...
    # ANSI fallback
    w = _cols(80)
    rule = Colors.SKY + "─" * w + Colors.RESET
    print(rule)
    for row in art:
        print("  " + _ansi(row, Colors.GOLD, Colors.BOLD))
    print("  " + _ansi(f"v{ver}  MINXG Chat — interactive REPL", Colors.SLATE))
    print(rule)
    blue_bg = Colors.bg("27") + Colors.WHITE + Colors.BOLD + Colors.RESET
    print(blue_bg + "  NO WARRANTY   —  AS IS, MIT License." + Colors.RESET)
    print(blue_bg + "  NO LEGAL ADVICE — EU 2009/24/EC Art.6 / DMCA §1201(f)." + Colors.RESET)
    print(blue_bg + "  DISCLAIMER OF AGENCY — You are the actor. Your liability." + Colors.RESET)
    print(rule)

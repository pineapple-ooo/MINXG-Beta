...
DANGEROUS_COMMANDS = frozenset({
    # POSIX destructive / privilege / power commands
    "rm", "del", "rd", "rmdir",
    "shutdown", "halt", "poweroff", "reboot", "init 0",
    "dd",
    "mkfs", "fdisk", "parted",
    ":(){ :|:& };:",
    "chmod 777", "chmod -r 777",
    "sudo", "su -",
    # Windows cmd.exe / PowerShell equivalents — cmd.exe is
    # case-insensitive and so is the `.lower()` comparison below.
    "format", "diskpart", "cipher /w", "vssadmin",
    "remove-item", "ri", "del /f", "del /s", "del /q",
    "rd /s", "rmdir /s",
...
def _is_dangerous_command(command: str) -> bool:
    ...
    for dangerous in ["rm", "del", "dd", "mkfs"]:
        if lower.startswith(dangerous + " ") and ("-rf" in lower or "-r " in lower or "-f" in lower):
            return True
    # PowerShell's Remove-Item honours -Recurse / -Force flags the same
    # way `rm -rf` does; catch those regardless of flag ordering.
    if lower.startswith(("remove-item ", "ri ")) and (
        "-recurse" in lower or "-force" in lower
    ):
        return True
    return False
...
def _handle_execute_command(args: dict) -> str:
    ...
    try:
        ...
        if os.name == "nt":
            # subprocess's default shell on Windows is already cmd.exe;
            # passing a POSIX `executable=` (e.g. the /bin/sh fallback
            # below) would make every command fail with
            # FileNotFoundError. Just let it use the platform default,
            # and don't force a Unix-only TERM value into the child env.
            run_kwargs["env"] = os.environ.copy()
        else:
            shell = os.environ.get("SHELL", "/bin/sh")
            run_kwargs["executable"] = shell
            run_kwargs["env"] = {**os.environ, "TERM": "xterm-256color"}

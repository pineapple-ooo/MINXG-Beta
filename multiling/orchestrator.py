...
class NexusOrchestrator:
    ...
    def __init__(
        ...
        self.ai_base_url = (ai_base_url or os.getenv("AI_BASE_URL") or ai_config.get("base_url", "http://localhost:11434/v1")).rstrip("/")
    ...
    async def _stream_conversation(self, messages, tools, session_id=None,
        ...
        # One session for every round-trip in this conversation, not a
        # fresh aiohttp.ClientSession() (and therefore a fresh TCP/TLS
        # handshake to the AI provider) per iteration — see
        # _run_with_upstream_ai's call_ai() for the same fix and the
        # full rationale.
                ...
                try:
                    async with session.post(
                        ...
                        if resp.status != 200:
                            ...
                            yield {"type": "error",
                                   "message": f"AI error {resp.status} at {self.ai_base_url}/chat/completions: {body[:200]}"}
                            return
    ...
    def _run_with_upstream_ai(self, messages: List[Dict], tools: List[Dict],
        ...
        async def call_ai(session, request_messages, request_tools):
            headers = {"Content-Type": "application/json"}
            if self.ai_api_key:
                headers["Authorization"] = f"Bearer {self.ai_api_key}"
        ...
        async def run_with_shared_session():
            # One session, reused across every tool-calling round-trip
            # in this conversation, instead of a fresh
            # aiohttp.ClientSession() (and therefore a fresh TCP/TLS
            # handshake to the AI provider) per round. Constructing a
            # session is cheap by itself; what it was actually costing
            # is connection reuse across calls that are, within one
            # conversation, all going to the exact same host.
            connector = aiohttp.TCPConnector(limit=10, keepalive_timeout=30)
            async with aiohttp.ClientSession(connector=connector) as session:
                async def bound_call_ai(request_messages, request_tools):
                    return await call_ai(session, request_messages, request_tools)
                return await self._async_run_conversation(
                    messages, tools, bound_call_ai, execute_tool, session_id=session_id)

        loop = asyncio.new_event_loop()
        try:
            result = loop.run_until_complete(run_with_shared_session())
            return result
        finally:
            loop.close()

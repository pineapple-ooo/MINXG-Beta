...
def ensure_tools_discovered():
    """Discover and import all builtin tools, plus wire up
    `register_hooks(registry)` on any *enabled* extension that defines
    one. Idempotent.
...
def _discover_extension_tools() -> None:
    """Call `register_hooks(registry)` on every *enabled* extension that
    defines one, passing this module's real `tools.registry.registry`.
    ...
    that wiring, gated on `ext.enabled` so a *disabled* extension's
    tools don't become callable just because its module happened to
    get imported for metadata purposes.

    Note: this does not retroactively make the pre-existing
    `register_hooks` implementations in files_ext/adb_ext/root_ext
    register real tools — they use a separate, still-decorative
    `tool_interceptor` metadata hook, not `registry.register()`
    directly, and changing that is a deliberate scope decision for
    each of those extensions to make on its own, not a side effect of
    fixing the fact that hooks weren't reachable at all.
    """
    try:
        from extensions.loader import get_extensions
    except ImportError:
        return
    try:
        extensions_list = get_extensions()
    except Exception as e:
        logging.getLogger(__name__).warning("Could not enumerate extensions: %s", e)
        return
    for ext in extensions_list:
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                "register_hooks failed for extension %r: %s", ext.name, e)

...
class Simplex:
    ...
    """
    vertices: FrozenSet[int]  # vertex IDs
...
class SimplicialComplex:
    ...
    _vertex_ids: Dict[int, str] = field(default_factory=dict)  # optional names
    ...
    def _add_recursive(self, simplex: Simplex) -> None:
        if simplex.dimension < 0:
            return  # don't add the empty set
    ...
    def boundary_matrix(self, k: int) -> List[List[int]]:
        ...
        for j, sigma in enumerate(k_simplices):
            for i, vertex in enumerate(sorted(sigma.vertices)):
                # The face obtained by removing the i-th vertex
    ...
    def betti_number(self, k: int) -> int:
        ...
        """
        # Use the formula: β_k = n_k - rank(∂_k) - rank(∂_(k+1))

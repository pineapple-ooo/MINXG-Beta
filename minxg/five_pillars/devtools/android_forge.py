"""minxg/five_pillars/devtools/android_forge.py — Heavy-weight Android forge worker.

The :class:`AndroidForgeWorker` gives the AI **first-class ability to
author, scaffold, validate, and build Android APKs**. It wraps the
canonical Buildozer / python-for-android toolchain, but layers on:
...
class AndroidForgeWorker(BaseWorker):
    ...
    * ``apk_status`` — what state is package.json in / last log?
    ...
    Plus, as of v0.18.0, an academic-use **APK reverse studio** chapter
    (apk_studio_inspect, _hash, _strings, _diff_manifests, _legal_notice)
    attached as 5 additional tool methods.  Every reverse call carries
    the legal-notice sibling field proving the academic-use boundary.
    """

    worker_id = "android_forge"
    version = "0.18.0"        # bumped for reverse-tools add-on
    ...
    async def apk_spec(self, root_path: str) -> Dict[str, Any]:
        ...
        spec_lines = [
            f"[app] {_BUILDOZER_PREFIX} auto-generated by AndroidForgeWorker",
...
# ─────────────────────────────────────────────────────────────────────────
# v0.18.0 — Reverse engineering chapter
# Caution: AndroidForgeWorker is for *building* Android apps.  It also surfaces
# a small set of reverse engineering tools to mirror what the AI will
# commonly want to know about an existing APK ("what is in this app?").
# The reverse tools are STRICTLY for academic / interoperability use as
# mandated by EU Directive 2009/24/EC Art. 6 and US DMCA §1201(f). They do
# NOT bundle encryption bypass, code decompilation, or DRM circumvention.
# Every reverse-call attaches `_apk_studio_notice` so the user must have
# already seen this disclosure before invoking anything.
# ─────────────────────────────────────────────────────────────────────────
...
_APK_STUDIO_NOTICE = (
    "ACADEMIC / INTEROPERABILITY-ONLY USE — by invoking this tool "
    "you confirm: (1) you lawfully possess a copy of the target app "
    "(license or purchase); (2) the analysis is for interoperability "
    "research, security audit, or academic study permitted under EU "
    "2009/24/EC Art. 6 or US DMCA §1201(f); (3) you will NOT bypass any "
    "DRM or redistribute copyrighted code; (4) any violation is your "
    "exclusive responsibility — MINXG and its contributors accept NO "
    "liability."
)


def _apk_studio_guard(reason: str, target: str) -> Dict[str, Any]:
    """Build the legal-notice sibling every reverse tool attaches."""
    return {
        "_apk_studio_notice": _APK_STUDIO_NOTICE,
...
def _apk_peek_manifest(xml_bytes: bytes) -> Dict[str, Any]:
    """Lightweight manifest peek — package / versionCode / versionName."""
    ...
    pkg = pkg_re.search(xml_bytes)
    if pkg:
        out["package"] = pkg.group(1).decode("ascii", "replace")
    for attr, key in ((b"versionCode", "version_code"),
                       (b"versionName", "version_name")):
        m = re.search(
            rb"%s[^\x00-\x1f]{0,32}=[\"']?([^\"'\x00-\x1f]{1,32})"
            % attr, xml_bytes)
        if m:
...
def _apk_peek_dex(dex_bytes: bytes) -> Dict[str, Any]:
    """DEX header counters (header-fields only)."""
    if len(dex_bytes) < 112 or dex_bytes[:4] != b"dex\n":
        return {"parsed": False, "reason": "missing DEX magic"}
    fields = struct.unpack_from("<10I", dex_bytes, 36)
    return {
        "parsed": True,
        "bytes": len(dex_bytes),
        "string_ids": fields[0],
        ...
        "method_ids": fields[4],
        "class_defs": fields[5],
        "scope": "structural counters only — bytecode not disassembled",
    }


# Insert reverse tools as additional @tool methods on AndroidForgeWorker.
# We do this by binding the @tool-decorated methods onto AndroidForgeWorker
# at module import.  The methods declare `_tool_meta` via the @tool
# decorator so BaseWorker._register_tools picks them up automatically
# when ``AndroidForgeWorker()`` is instantiated.

_APK_FORGE_REVERSE_TOOL_COUNT = 5
...
@tool(
    description=(
        "Static, read-only structural inspection of an APK.  Returns "
        "manifest fields (package/versionCode/versionName), DEX "
        "header counters, and resource category enumeration.  Does NOT "
        "decompile bytecode or bypass DRM.  Academic / "
        "interoperability use only — see attached legal notice."
    ),
    category="analyze",
)
async def apk_studio_inspect(self, apk_path: str) -> Dict[str, Any]:
    p = Path(apk_path).expanduser()
    if not p.exists():
        return {"status": "error", "error": f"{p} not found",
                **_apk_studio_guard("refused — bad input", apk_path)}
    try:
        with _zipfile.ZipFile(p) as zf:
            manifest = _apk_peek_manifest(zf.read("AndroidManifest.xml"))
            dex_files = [n for n in zf.namelist() if n.endswith(".dex")]
            dex_info: Dict[str, Any] = {}
            if dex_files:
                dex_info = _apk_peek_dex(zf.read(dex_files[0]))
                dex_info["file_count"] = len(dex_files)
            resources = sorted({n.split("/")[1] for n in zf.namelist()
                                 if n.startswith("res/") and "/" in n})
        return {
            "status": "ok",
            "path": str(p),
            ...
            "manifest": manifest,
            "dex": dex_info,
            "resources_categories": resources,
            **_apk_studio_guard(
                "static structural summary — no bytecode decode",
                apk_path),
        }
...
# ── hash (auditing / accountability) ─────────────────────────────────────

@tool(
    description=(
        "sha256 of an APK.  Useful for accountability logs when "
        "running academic research; the user is responsible for "
        "ensuring they have lawful access to the file before invoking."
    ),
    category="analyze",
)
async def apk_studio_hash(self, apk_path: str) -> Dict[str, Any]:
    p = Path(apk_path).expanduser()
    if not p.exists():
        return {"status": "error", "error": f"{p} not found",
                **_apk_studio_guard("refused — bad input", apk_path)}
    ...
    return {"status": "ok", "path": str(p),
             "sha256": h.hexdigest(),
             "size_bytes": p.stat().st_size,
             **_apk_studio_guard("sha256 — proof of artefact", apk_path)}
...
@tool(
    description=(
        "List printable ASCII strings ≥4 chars from a binary file.  No "
        "decryption.  Commonly used to surface version strings, public "
        "URLs, and permission owners for interoperability research — "
        "the user is responsible for lawful access."
    ),
    category="analyze",
)
async def apk_studio_strings(
    ...
    return {"status": "ok", "path": str(p),
             ...
             "strings": hits,
             **_apk_studio_guard("printable strings scan", apk_path)}
...
@tool(
    description=(
        "Compare two manifest summaries (as returned by "
        "apk_studio_inspect).  Returns added/removed/changed keys.  Use "
        "this for **interoperability / version compatibility research** "
        "only — not for piracy."
    ),
    category="analyze",
)
async def apk_studio_diff_manifests(
    self, manifest_a: Dict[str, Any], manifest_b: Dict[str, Any],
) -> Dict[str, Any]:
    if not isinstance(manifest_a, dict) or not isinstance(manifest_b, dict):
        return {"status": "error",
                "error": "manifest_a and manifest_b must be dicts",
                **_apk_studio_guard("refused — bad input",
                                     "comparison input")}
    ...
    return {"status": "ok",
             ...
             "removed": sorted(r),
             "changed": sorted(c),
             **_apk_studio_guard("interoperability comparison",
                                  "two manifest summaries")}
...
@tool(
    description=(
        "Print the academic-use / liability disclosure that "
        "apk_studio_* tools attach.  Call this once before invoking "
        "any other apk_studio_* tool so the user has explicitly seen "
        "the boundary."
    ),
    category="analyze",
)
async def apk_studio_legal_notice(self) -> Dict[str, Any]:
    return {"status": "ok", "legal_notice": _APK_STUDIO_NOTICE}


# ── register all six tools onto AndroidForgeWorker ────────────────────────────

for _name in ("apk_studio_inspect", "apk_studio_hash",
              "apk_studio_strings", "apk_studio_diff_manifests",
              "apk_studio_legal_notice"):
    setattr(AndroidForgeWorker, _name, locals()[_name])

# Backward-compat alias — callers that had been importing
# ``ApkForgeWorker`` (the v0.17.x name) keep working unchanged.
ApkForgeWorker = AndroidForgeWorker

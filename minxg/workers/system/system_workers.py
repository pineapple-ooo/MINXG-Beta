...
class ProcessWorker:
    ...
    def execute(self, operation: str = "list", pid: Optional[int] = None,
        ...
        if operation == "list":
            ...
            try:
                if os.name == "nt":
                    # signal.SIGTERM on Windows maps to TerminateProcess()
                    # under the hood (there's no graceful-signal delivery
                    # to an arbitrary process on Windows), so this is
                    # already the best available "kill" there.
            ...
            try:
                if os.name == "nt":
                    # signal.SIGKILL doesn't exist on Windows at all; the
                    # actual force-kill equivalent is `taskkill /F`.

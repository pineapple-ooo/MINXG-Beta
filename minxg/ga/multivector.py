...
def blade_sign(a: int, b: int) -> int:
    ...
    """
    if a & b:
        return 0  # overlapping basis -> zero (for 0-Clifford, if not grade-2)
    s = 0
...
def blade_outer(blades_a: int, blades_b: int) -> int:
    ...
    if blades_a & blades_b:
        return 0  # wedge of overlapping blades is 0
...
class Multivector:
    ...
    def norm(self) -> float:
        ...
        n2 = self.norm_sq
        if n2 < 0:
            return math.sqrt(-n2)  # indefinite metric
        return math.sqrt(n2)
    ...
    def exp(self) -> "Multivector":
        ...
        # Use Taylor series, terminate at 20 terms
        result = Multivector.scalar(1.0, self.sig)
    ...
    def log(self) -> "Multivector":
        ...
        """
        # Project to grade-0 + grade-2 (bivector) for canonical log
        s = self[0]
        B = self.grade(2)
        B_sq = geometric_product(B, B)[0]  # B·B
        b_norm = math.sqrt(max(0, -B_sq))  # may be 0
        if b_norm < 1e-12:
            # commuting case: log = ln(s) + B/s
            if s <= 0:
                raise ValueError("Multivector log undefined for non-positive scalar part")
            return Multivector({0: math.log(s)}, self.sig) + B / s
        # Use B / b_norm = unit bivector; then log(M) = ln(|M|) + θ * (B / b_norm)

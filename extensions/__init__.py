...
def register_cli_extensions(subparsers) -> Dict[str, ExtensionModule]:
    ...
    command dispatch is returned in the ext_map keyed by the *actual*
    ...
    extension command would silently fall through to whatever the
    top-level parser does with an unrecognized command. Diffing
    ``subparsers.choices`` before/after each extension's own
    registration call finds the real verb(s), however many an
    extension adds, without needing every extension to declare a
    separate "this is my CLI verb name" field just to make dispatch
    work.
    """
    ...
    try:
        for ext in get_extensions():
            if not ext.enabled:
                continue
            before = set(getattr(subparsers, "choices", {}) or {})
            ext.register_cli(subparsers)
            after = set(getattr(subparsers, "choices", {}) or {})
            for verb in after - before:
                ext_map[verb] = ext
    except Exception as e:

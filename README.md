...
Six categorical libraries ship with the package and register **306
mathematical operator IDs** in 6 non-overlapping ranges on import;
`OPERATOR_REGISTRY` exposes **376 operators total** across **11
categories** (see "Runtime truth" below for the canonical list):

```
minxg.ga         5000-5046   geometric algebra       (47 ids)
...
The 11 categories in `OPERATOR_REGISTRY` (`cat`,
`chaos`, `data`, `fiber`, `ga`, `infogeo`, `logic`, `math`, `system`,
`text`, `topo`) hold **376 operator IDs total**, of which the 6 mathematical
pillars above contribute the **306 mathematical operator IDs**. To dump
the full list at runtime:

```python
from minxg.operators import OPERATOR_REGISTRY
print(OPERATOR_REGISTRY.total_operators)            # 376
print(OPERATOR_REGISTRY.list_categories())          # 11 categories
for cat in OPERATOR_REGISTRY.list_categories():
    print(cat, len(OPERATOR_REGISTRY.get_category(cat)))
```

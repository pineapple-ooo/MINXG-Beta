...
class TestEscapeMarkup:
    def test_escapes_square_brackets(self):
        ...
        # Only `[` needs escaping — that's the character that starts a
        # rich markup tag. `]` alone isn't meaningful to rich's parser;
        # escaping it too used to leave a literal stray backslash in
        # front of every `]` once rich rendered it (rich only
        # recognizes `\[` as a real escape sequence, not `\]`).
        assert "\\[" in out
        assert "\\]" not in out
        assert "]" in out  # the closing bracket itself is untouched

    def test_escapes_backslashes(self):
        raw = "path\\to\\[file]"
        out = utils_mod._escape_markup(raw)
        # Backslashes preserved; only `[` gets escaped
        assert "\\[" in out
        assert "to" in out
    ...
    def test_rendered_output_has_no_stray_backslash(self):
        """End-to-end check against the actual bug this was fixed for:
        render `[category] name` through rich and confirm no literal
        backslash shows up next to the closing bracket."""
        import io
        from rich.console import Console
        raw = "[development] my-skill"
        escaped = utils_mod._escape_markup(raw)
        buf = io.StringIO()
        console = Console(file=buf, force_terminal=False, width=200)
        console.print(f"[green]{escaped}[/green]")
        rendered = buf.getvalue()
        assert "\\]" not in rendered
        assert "[development] my-skill" in rendered

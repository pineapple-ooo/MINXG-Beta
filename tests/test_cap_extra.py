...
def test_cli_cap_check_returns_zero_on_clean_tree_subprocess(tmp_path):
    ...
    script.write_text(textwrap.dedent(inner))
    # The project is not pip-installed in some dev environments, so make
    # the project root importable for the child interpreter.
    env = os.environ.copy()
    extra = str(Path(__file__).resolve().parent.parent)

    ...
    def test_api_key_from_args_over_config(self, tmp_path):
        ...
        try:
            ...
            with mock.patch("multiligua_cli.main.load_config", return_value={
                ...
                # start_api_server is imported locally inside run_open(), so
                # it must be patched at its definition site. Patching it
                # too (not just asyncio.run) matters: `asyncio.run(x)`
                # evaluates `x = start_api_server(...)` first regardless of
                # asyncio.run being mocked, so leaving the real coroutine
                # function in place here creates a genuine, never-awaited
                # coroutine object that only gets garbage-collected (and
                # warns) much later, inside some unrelated test.
                with mock.patch(
                    "multiling.orchestrator.start_api_server", new=lambda **kw: None
                ):
                    with mock.patch(

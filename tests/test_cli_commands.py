...
class TestGateway:
    ...
    def test_gateway_no_subcommand_runs_foreground(self):
        ...
        the (unreachable, in tests) py-worker connection eventually gave
        up after ~60s and let the process exit. Under any extra load
        that windUp could blow past pytest's timeout and hang instead of
        failing cleanly. `run_gateway` is mocked the same way
        ``test_cli_gateway.py::TestGatewayForeground`` already does it,
        so this stays a fast, deterministic check that the dispatcher
        reaches the foreground path and handles both the interrupt and
        the error branch without raising.
        """
        with mock.patch("gateway.runner.run_gateway", side_effect=KeyboardInterrupt):
            rc, _ = _silent_call(["gateway"])
        assert rc == 0, f"KeyboardInterrupt should map to rc=0, got {rc}"

        with mock.patch("gateway.runner.run_gateway", side_effect=RuntimeError("boom")):
            rc, _ = _silent_call(["gateway"])
        assert rc == 1, f"a real startup error should map to rc=1, got {rc}"

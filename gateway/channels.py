...
A *channel* is one inbound surface the gateway listens to. The default
``rest`` channel is the aiohttp server itself; this module adds optional
``memory`` and ``http`` channels so the same gateway can receive messages
from internal loops or webhook-style HTTP callbacks without coupling the
senders to the OpenAI v1 API shape.
"""

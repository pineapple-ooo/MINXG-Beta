# MINXG Architecture (v0.18.5)

> Think of this as the "how does this thing actually work?" doc.
> Written for developers who want to ship.
...
| Pillar | Does | Description |
|--------|------|-------------|
| `scalar` | Math + text ops | Pure functions over values, no side effects |
| `aggregate` | Encoding, crypto, ML templates | Bundles values, encodes, transforms bulk data |
| `io` | FS, network, web | Crosses the process boundary |
| `dispatch` | Shell, ADB, sysadmin stuff | Invokes a system or controls another process |
